...
describe('Server Observer B — error recovery paths', () => {
  ...
  test('outer-catch recovery on a beyond-tolerance doc clears witness coherence: next in-sync fragment edit does not run a cross-generation residual merge', () => {
    ...
    // Beyond-tolerance fixture (an over-indented list continuation
    // canonicalizes to the two-space continuation indent), so a coherent
    // witness pair WOULD qualify for the in-sync residual merge.
    const ngRaw =
...
describe('Observer A routing — Path B fires iff Y.Text holds unabsorbed changes (FR-3)', () => {
  ...
  // Beyond-tolerance fixture: an over-indented list continuation line
  // canonicalizes to the two-space continuation indent — a byte divergence
  // normalizeBridge does NOT tolerate. Storage never sanitizes: these bytes
  // must survive in-sync fragment settlements. (Prior fixtures — inline-math
  // `$a + b$`, the lazy blockquote continuation, then a literal NBSP — were
  // each retired when the engine grew byte-faithful for them.)
  const NG_RAW =
