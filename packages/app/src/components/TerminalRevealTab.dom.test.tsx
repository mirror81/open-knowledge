...
describe('TerminalRevealTab', () => {
  ...
  test('exposes an "Open session dock" control and fires onReveal on click', async () => {
  ...
  test('marks which edge it hugs so it sits where the collapse control was', () => {
    ...
    expect(
      screen
        .getByRole('button', { name: 'Open session dock' })
        .getAttribute('data-terminal-reveal'),
    ).toBe('right');
    cleanup();
  ...
  test('surfaces its label in a tooltip on hover', async () => {
    ...
    await user.hover(screen.getByRole('button', { name: 'Open session dock' }));

    const tooltip = await screen.findByRole('tooltip');
    expect(tooltip.textContent).toContain('Open session dock');
  });
});
